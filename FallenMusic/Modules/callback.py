...
async def admin_cbs(_, query: CallbackQuery):
    ...
    if data == "resume_cb":
        ...
        if not await is_streaming(query.message.chat.id):
            return await query.answer(
                "◍ التشغيل موقف مؤقتا مسبقا", show_alert=True
            )
        await stream_off(query.message.chat.id)
        await pytgcalls.pause_stream(query.message.chat.id)
        await query.message.reply_text(
            text=f"➻ تم ايقاف الموسيقي مؤقتا \n│ \n└ʙʏ : {query.from_user.mention} ",
            reply_markup=close_key,
        )
        ...
        if not get:
            try:
                ...
                await pytgcalls.leave_group_call(query.message.chat.id)
                await query.message.reply_text(
                    text=f"➻ **__تم ايقاف__** \n│ \n└ʙʏ : {query.from_user.mention} \n\n**__◍ لا يوجد شي بعد هذي الاغنيه في__** {query.message.chat.title}",
                    reply_markup=close_key,
                )
...
async def unban_ass(_, CallbackQuery):
    ...
    if umm.can_restrict_members:
        try:
            ...
            return await CallbackQuery.answer(
                "◍ فشل في الغاء حظر حساب المساعد √",
                show_alert=True,
            )
        return await CallbackQuery.edit_message_text(
            f"➻ {ASS_NAME} تم الغاء حظر حساب المساعد {CallbackQuery.from_user.mention}.\n\nيمكنك التشغيل الان..."
        )
    else:
        return await CallbackQuery.answer(
            "◍ ليس لدي صلاحيه الغاء حظر √",
            show_alert=True,
        )
...
async def help_menu(_, query: CallbackQuery):
    ...
    try:
        await query.edit_message_text(
            text=f"""**__◍ هذي هي اقسام الاوامر ↓

◍ اهلا بك عزيزي في سورس فينوم__**""",
